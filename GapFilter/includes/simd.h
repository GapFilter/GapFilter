...
public:
    ...
    int Insert(uint16_t visitnum)
    {
        ...
        int16_t *p = (int16_t*)&m64_diff;
        for(int i = 0; i < 4; i++){
            if(abs(p[i]) < abs(min_diff)){
                min_diff = p[i];
                matchplace = i;
            }
        }
...
public:
    ...
    {
        uint32_t tailzero = -1;
        uint32_t matchplace = -1;                                               
        uint32_t res = 0;                                                  
        __m128i m128i_visitfp = (__m128i)_mm_set1_epi16(fp);               
        __m128i m128i_eqres = _mm_cmpeq_epi16(m128i_visitfp, m128i_num_fp);
        __mmask8 eqmask =  _mm_cmp_epi16_mask(m128i_visitfp, m128i_num_fp, _MM_CMPINT_NE);

        __m128i m128i_visitseq, m128i_diff, m128i_absdiff, 
                high_part_4, min_4, high_part_2, min_2, high_part_1, min_1;
        __m128i m128i_mindiff, m128i_eqdiffres;
        uint16_t min_absdiff;
        int16_t min_diff;
...
nomatch: 
        matchplace = _tzcnt_u32(_mm_movemask_epi8(_mm_and_si128(
            _mm_cmpeq_epi16(m128i_num_seq, __m128izreo), _mm_cmpeq_epi16(m128i_num_fp, __m128izreo)))) / 2;
        if (matchplace < sz)
        {
            ((uint16_t *)&m128i_num_seq)[matchplace] = seq;
            ((uint16_t *)&m128i_num_fp)[matchplace] = fp;

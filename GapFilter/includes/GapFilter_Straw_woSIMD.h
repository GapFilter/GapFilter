#ifndef _GAPFILTER_STRAW_WOSIMD_H
#define _GAPFILTER_STRAW_WOSIMD_H
...
class GapFilter_Straw_woSIMD : public GapFilter
{
public:
    struct cell{
...
public:
    GapFilter_Straw_woSIMD(int memory, int hash_seed = 1000)
        : GapFilter(memory)
    {
...
find:
    ...
    string Name()
    {
        return string("GapFilter_Straw_woSIMD");
    }
    ~GapFilter_Straw_woSIMD()
    {
        for(uint64_t i = 0; i < bucket_num; i++)
            delete[] table[i];
        delete[] table;
